...
### 2026-08-12 — 자매 프로젝트 `nl-openapi-mcp` 세션에서 이관된 교훈
...
- ✅ **적대적 검증 실제 HTTP 왕복 — 이식 완료, 결함 3종 검출(2026-08-12)**
  `tests/test_adversarial_http.py`(67 → **86**). 🔴 토큰 응답 **본문 전체**를 예외에 담아
  `refresh_token`·`client_id` 가 `scienceON_status` 응답까지 누출 · 🔴 charset 헤더 부재 시
  한글 깨짐 · 🔴 `rows=1`·`max=300` 이 300회 요청.
  ⚠️ 누출 검사가 과해서 정상 전달까지 막지 않았는지 보는 **대조군**을 같이 넣었다.
- ⚠️ **자격증명 누출은 전송 계층 밖에서도 난다** — kci 에서 확인(2026-08-12).
  그 저장소는 `raise_for_status()` 를 처음부터 피했는데도, **오류 응답이 요청을 에코하고**
  파서가 전 요소 텍스트를 이어붙이는 바람에 **인증키가 MCP 응답까지 샜다.**
  ⚠️ 더 나쁜 것은 `_safe` 독스트링이 "`_call` 단계에서 이미 제거되므로 안전하다"고
  **틀린 보증**을 하고 있었다는 점이다 — 그 보증을 믿으면 상류를 검사하지 않게 된다.
  → 이 저장소도 **응답 본문을 사용자에게 되돌리는 경로**(파서 예외·XML 오류 메시지)를
    별도로 점검할 것. 전송 계층을 잠갔다는 사실은 근거가 되지 않는다.
- ⚠️ **경고 문자열은 안전장치인데 테스트가 잘 닿지 않는다** — nl 에서 분할 수집 경고가
  **잘못된 키 이름 하나** 때문에 한 번도 출력되지 않았다. '전수가 아니다'를 알리는 문장이
  사라졌는데도 테스트는 전부 통과했다. 이 저장소의 `search_*_meta` 경고들도 회귀로 고정할 것.
- **대리 지표를 결론으로 쓰지 말 것** — nl 에서 '오탐률 52%'를 근거로 구문검색을 기본값으로
  올리려 했으나, 그 지표는 *제목에 문자열이 있는가* 였지 *주제가 무관한가* 가 아니었다.
  실제로 버려지는 레코드를 확인하니 76%가 관련 문헌이어서 결론이 정반대가 됐다.
  → 이 저장소의 `contains` 필터·와일드카드 재현율을 평가할 때도 같은 함정에 주의.
- **부재 증명에는 대조군이 필요하다** — "검색 결과 0건이니 미지원"이라 단정했다가 뒤집었다.
  질의 자체가 부적절해 0건이었던 것. 미지원 판정 전에 반드시 성공하는 대조 질의를 둘 것.
